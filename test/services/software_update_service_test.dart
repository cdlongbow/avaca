...
void main() {
  group('SemanticVersion', () {
    test('compares release tags numerically', () {
      expect(
        SemanticVersion.parse('v0.7.10').compareTo(
          SemanticVersion.parse('0.7.9'),
        ),
        greaterThan(0),
      );
      expect(SemanticVersion.parse('0.7.10'), SemanticVersion.parse('v0.7.10'));
      expect(() => SemanticVersion.parse('v0.7.10-beta'), throwsFormatException);
    });
  });

  group('SoftwareUpdateService', () {
    test('selects the exact Android asset and verifies its streamed digest', () async {
      final bytes = Uint8List.fromList(List<int>.generate(128, (index) => index));
      final checksum = sha256.convert(bytes).toString();
      final tempRoot = await Directory.systemTemp.createTemp('avaca-service-test-');
      ...
      final client = MockClient((request) async {
        ...
        return http.Response('$checksum  avaca-0.8.1-arm64-v8a.apk\n', 200);
      });

      final service = SoftwareUpdateService(
        client: client,
        versionProvider: _FakeVersionProvider(_androidVersion('0.8.0')),
      ...
      expect(result.asset?.name, 'avaca-0.8.1-arm64-v8a.apk');
    ...
    test('fails closed without a compatible asset or source archive fallback', () async {
      final client = MockClient((request) async {
        return http.Response(
          jsonEncode(_releaseJson(
            ...
            assets: [
              ...
              _asset('avaca-0.8.1-x86_64.apk', 4),
            ],
          )),
          200,
        );
    ...
    test('removes a partial download after digest mismatch', () async {
      ...
      final client = MockClient((request) async {
        ...
        if (request.url.path.endsWith('.apk')) return http.Response.bytes(bytes, 200);
...
Map<String, dynamic> _releaseJson({
  ...
  final releaseAssets = assets ?? [
    _asset('avaca-0.8.1-arm64-v8a.apk', size, digest: checksum),
    _asset('avaca-0.8.1-arm64-v8a.apk.sha256', 70),
  ];
  return {
    'tag_name': 'v0.8.1',
    'draft': false,
...
Map<String, dynamic> _asset(String name, int size, {String? digest}) {
  return {
    'name': name,
    'browser_download_url': 'https://github.com/william12233/avaca/releases/download/v0.8.1/$name',
    'size': size,
